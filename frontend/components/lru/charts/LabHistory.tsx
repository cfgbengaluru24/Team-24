...
import { fetchLabHistory } from "@/lib/data";
...
export default function LabHistStats() {
  const { isPending, error, data } = useQuery<Data[]>({
    queryKey: ["lab_history_stats"],
    queryFn: async () => {
      const data = await fetchLabHistory();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Labourers Workhours</CardTitle>
      </CardHeader>
      <CardContent>
